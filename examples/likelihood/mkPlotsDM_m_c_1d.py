...
def mkSPPostCmdPL1D(datafile, ix, lx, no_x=False, plot_lim=None):
    cmd = "python -m superplot.super_command %s --xindex=%i  --show_prof_like=1  --plot_description='One-dimensional plot.' --show_posterior_mode=0 --show_posterior_mean=0 --show_posterior_pdf=0 --show_posterior_median=0 --show_credible_regions=0 --show_conf_intervals=0"%(datafile, ix)
    cmd+= " --kde_pdf=1 --show_best_fit=1"
    cmd+=" --leg_position='no legend'"
    if no_x:
        cmd+= " --xlabel=noxlabel"
    ...
    cmd+= " --output_file=plot_%i_profile_xlabel%i.pdf"%(ix, 1-int(no_x))
    return cmd
...
for numx, x in enumerate(LABELS):
    ...
    CMDS.append(mkSPPostCmdPL1D(DATA, numx+2, x, plot_lim=ploli))
